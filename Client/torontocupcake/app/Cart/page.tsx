...
export default function page() {
    ...
    const handleModalContinuePressed = (event: React.FormEvent<HTMLFormElement>) => {
        event.preventDefault()
        console.log("data saved")
        saveAddressToLocalStorage(addressData)

        //  YAHA AGR STRIPE MA DATA BHEJNA TO HAMEN IS FORM MA CHAIYE oki oki ...saii
        //  ye subtotal wali cheez kaha ha?
        // [{
        //     id: 1,
        //     quantity: 3
        // },
        ...
        // sirf item ki id aur quantity
        //  OPEN THE STRIPE INTERFACE IF NOT CASH ON DELIVERY
        //  i  
    }
    ...
    useEffect(() => {
        ...
        setTotalDeliveryCharge(calculateDeliveryCharge(totalSum))}    , [cartItems])

    return (
        <section className="py-24 relative">
            <div className="w-full max-w-7xl px-4 md:px-5 lg-6 mx-auto">

                <h2 className="title font-manrope font-bold text-4xl leading-10 mb-8 text-center font-averia text-[#3D0C11]">Your Cart
                </h2>
                ...
                <div className="bg-gray-50 rounded-xl p-6 w-full mb-8 max-lg:max-w-xl max-lg:mx-auto">
                    <div className="flex items-center justify-between w-full mb-6">
                        <p className="font-normal text-xl leading-8 text-[#3D0C11]">Sub Total</p>
                        <h6 className="font-semibold text-2xl leading-8 text-[#3D0C11]">${subTotal.toFixed(2)}</h6>
                    </div>
                    <div className="flex items-center justify-between w-full pb-6 border-b border-gray-200">
                        <p className="font-normal text-xl leading-8 text-[#3D0C11]">Delivery Charge</p>
                        <h6 className="font-semibold text-2xl leading-8 text-[#3D0C11]">${totalDeliveryCharge.toFixed(2)}</h6>
                    </div>
                    <div className="flex items-center justify-between w-full py-6">
                        <p className="font-marcellus font-medium text-2xl leading-9 text-[#3D0C11]">Total</p>
