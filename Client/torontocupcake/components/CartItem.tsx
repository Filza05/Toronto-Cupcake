...
const CartItem: React.FC<CartItemProps> = ({item, handleDelete, cartItems, setCartItems}) => {
        ...
        <div className='font-averia'>
            <div className="grid grid-cols-1 lg:grid-cols-2 min-[550px]:gap-6 border-t border-gray-200 py-6">
                <div
                    ...
                    <div className="img-box"><img src={item.imageSrc} alt="perfume bottle image" className="xl:w-[140px]" /></div>
                    <div className="pro-data w-full max-w-sm ">
                        <h5 className="font-semibold text-2xl leading-8 text-[#3D0C11] max-[550px]:text-center">
                            {item.name}
                        </h5>

                        <h6 className="font-medium text-xl leading-8 text-[#D14D72] max-[550px]:text-center">$ {item.price} per cupcake</h6>
                    </div>
                </div>
                <div
                    ...
                    $ {deliveryCharge.toFixed(2)} <span className="text-sm text-black ml-3 lg:hidden whitespace-nowrap">(Delivery
                            Charge)</span></h6>
                    <div className="flex items-center w-full mx-auto justify-center">
                        <button 
                            onClick={() => {handleItemQuantityDecrement(item.id)}}
                        ...
                        <button
                            ...
                            <svg className="stroke-gray-900 transition-all duration-500 group-hover:stroke-black"
                                ...
                                <path d="M11 5.5V16.5M16.5 11H5.5" stroke="" strokeWidth="1.6"
                                    strokeLinecap="round" />
                                <path d="M11 5.5V16.5M16.5 11H5.5" stroke="" stroke-opacity="0.2" strokeWidth="1.6"
                                    strokeLinecap="round" />
                                <path d="M11 5.5V16.5M16.5 11H5.5" stroke="" stroke-opacity="0.2" strokeWidth="1.6"
                                    strokeLinecap="round" />
                            </svg>
                    ...
                    <h6
                        className="text-[#3D0C11] font-manrope font-bold text-3xl leading-9 w-full max-w-[176px] text-center">
                        {(quantity * item.price).toFixed(2)}</h6>
                </div>
                <button
                    ...
                    Delete 
                    <svg className="w-6 h-6 ml-1 text-white group-hover:text-[#3D0C11]" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
